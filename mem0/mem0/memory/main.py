                ...
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        if memory_id is None:
                            logger.warning(
                                "Skipping UPDATE: id %s not in current memory list (valid: %s). LLM may have hallucinated id.",
                                resp.get("id"),
                                list(temp_uuid_mapping.keys()),
                            )
                        else:
                            self._update_memory(
                                memory_id=memory_id,
                        ...
                        if memory_id is None:
                            logger.warning(
                                "Skipping DELETE: id %s not in current memory list (valid: %s). LLM may have hallucinated id.",
                                resp.get("id"),
                                list(temp_uuid_mapping.keys()),
                            )
                        else:
            ...
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    if event_type == "ADD":
                        ...
                        if memory_id is None:
                            logger.warning(
                                "Skipping UPDATE (async): id %s not in current memory list (valid: %s). LLM may have hallucinated id.",
                                resp.get("id"),
                                list(temp_uuid_mapping.keys()),
                            )
                        else:
                        ...
                        if memory_id is None:
                            logger.warning(
                                "Skipping DELETE (async): id %s not in current memory list (valid: %s). LLM may have hallucinated id.",
                                resp.get("id"),
                                list(temp_uuid_mapping.keys()),
                            )
                        else:
