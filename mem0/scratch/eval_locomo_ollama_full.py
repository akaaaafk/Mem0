# Usage:
# 用法：
#   python eval_locomo_ollama_full.py --method both --data mem0/evaluation/locomo/locomo10.json --outdir ./results/eval_locomo_ollama --max-samples 1 --max-questions 5 --verbose
...
def invoke_ollama(base_url: str, model: str, prompt: str, max_tokens: int = 256) -> tuple[str, int, int]:
...
def run_search(
    ...
    for conv_idx, item in enumerate(data):
        ...
        for q in qa_list:
            ...
            response_text, _, _ = invoke_ollama(OLLAMA_BASE_URL, OLLAMA_LLM_MODEL, prompt)
    ...
    print()
    print("=" * 60)
    print("LoCoMo mem0 完整测评（全部 Ollama）")
    print("=" * 60)
    print(f"  问题数: {n}")
            ...
            json.dumps(
                {
                    ...
                    "avg_f1": avg_f1,
                    "avg_bleu1": avg_bleu1,
                    "experiment_time_seconds": round(end_time - start_time, 2),
                },
                ensure_ascii=False,
            )
            + "\n"
...
def main():
    parser = argparse.ArgumentParser(description="LoCoMo mem0 测评 — 全部 Ollama")
    parser.add_argument("--method", choices=["add", "search", "both"], default="both")
    parser.add_argument("--data", type=str, default=None)
    parser.add_argument("--outdir", type=str, default="./results/eval_locomo_ollama")
